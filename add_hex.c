#include "main.h"
/**
 *add_hex - converts a number to hexadecimal and prints it
 ...
 *Return: buffer_index
 */
int add_hex(unsigned long int num, char *buffer, int buffer_index,
		      char c)
{
	...
	n = num;
	if (n / 16 != 0)
		buffer_index = add_hex(n / 16, buffer, buffer_index, c);
	r = n % 16;
	if (r < 10)
